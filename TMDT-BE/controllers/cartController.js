                ...
                { $match: { user: user._id } },
                {
                    $lookup: {
                        ...
                        pipeline: [
                            ...
                            { $unwind: { path: "$store_id", preserveNullAndEmptyArrays: true } },
                            {
                                $lookup: {
                                    from: "cartitems",
                                    let: { csid: "$_id" },
    ...
    addToCart: async (req, res) => {
        try {
            ...
            if (cart_item) {
                ...
                cart_item = await CartItemModel.create({ cartStore_id: cart_store._id, variant_id: variant_id, quantity: quantity, unitPrice: product.price })
            }
    ...
    buyNow: async (req, res) => {
        try {
            ...
            if (cart_item) {
                ...
                cart_item = await CartItemModel.create({ cartStore_id: cart_store._id, variant_id: variant_id, quantity: quantity, unitPrice: product.price, is_chosen: true })
            }
            res.status(200).send({ message: "Item added to cart", cart, id: cart_item._id });
        } catch (error) {
            console.error(error);
        ...
        try {
            ...
            await Promise.all(
                cartStore.map(async (s) => {
                    const num = await CartItemModel.countDocuments({ cartStore_id: s._id })
                    itemCount += num
                })
            )
